var time = 0;
var audio = new Audio('audio/hbd8bit.mp3');
function Gravity(id){
  var that = this;
  ...
  this.floatBackwards = function(){
    this.textSpans.forEach(function(t,i){
      ...
      var x = that.lerp(t._own.pos.x,0, that.resetTime / 50);
      var y = that.lerp(t._own.pos.y,0, that.resetTime / 50);

      t.style.transform = 'translateX('+ x +'px) translateY('+ y +'px)';

    });

    if(this.resetTime===50){
      this.animate = false;
      this.resetTime = 0;
    }
    this.resetTime++;
...
button.addEventListener('click',function(){
  if(paragraph.animate){
    ...
    paragraph.reset();
    audio.play();
  $("body .cards .card .col:nth-child(2)").css({"background":"url('https://pbs.twimg.com/media/Bqq08LgCUAAh-_Y.jpg')","background-size":"cover","background-repeat":"no-repeat"});
  }else{
    //gravity.restart();
    paragraph.restart();
    $("body .cards .card .col:nth-child(2)").css({"background":"url('https://pbs.twimg.com/media/Bq09x2VCMAEJOLX.jpg')","background-size":"cover","background-repeat":"no-repeat"});
  }
});
